 ...
 * @param {number} [props.cardsPerRow=3] - Number of cards to display per row (3 or 4).
 */
const InvitationGrid = ({ title, invitations, cardsPerRow = 3 }) => {
  if (!invitations || invitations.length === 0) {
    return null;
  }

  const gridClass = cardsPerRow === 4 ? styles.gridFour : styles.gridThree;
